...
const CircleCard = ({ groupId, title, subtitle, tasks, checkedInToday, color }: CircleCardProps) => {
  const style = styles[color];
  const Icon = style.Icon;

  return (
    <Link to={`/circles/${groupId}`} className="block">
      <motion.div
        whileHover={{ y: -5 }}
        transition={{ duration: 0.25 }}
        className="cursor-pointer rounded-3xl border border-slate-200 bg-white p-5 shadow-sm transition-all duration-300 hover:shadow-lg"
      >
